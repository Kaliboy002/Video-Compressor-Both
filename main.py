...
async def compress_video(_, message):
    ...
    if selected_format:
        ...
        async def compress():
            ...
            # Upload the compressed video to the specified download directory
            download_directory = user_data[user_id].get('download_directory')
            if download_directory:
                compressed_video_path = os.path.join(download_directory, 'output_video.mp4')
                os.rename('output_video.mp4', compressed_video_path)
                await app.send_video(user_id, compressed_video_path)
                await app.send_message(user_id, f"Compressed video saved to {compressed_video_path}")
            
            user_data[user_id] = {}
        
        await asyncio.create_task(compress())
    else:
        await message.reply_text("Please start the bot and select a format before sending the video.")

# Function to automatically select a format based on video resolution
def auto_select_format(message):
    video = message.video
    width = video.width
    height = video.height
...
if __name__ == "__main__":
    formats = {
        '1080p': ['-s', '1920x1080', '-b:v', '4000k', '-c:v', 'libx264', '-c:a', 'aac', '-b:a', '192k'],
        '720p': ['-s', '1280x720', '-b:v', '2500k', '-c:v', 'libx264', '-c:a', 'aac', '-b:a', '192k'],
        '360p': ['-s', '640x360', '-b:v', '800k', '-c:v', 'libx264', '-c:a', 'aac', '-b:a', '128k'],
    }
